...
def _promote_reg_cons(ctx:PreRegAllocContext, buf:UOp) -> tuple[Register, ...]:
  ...
  Flash output acc (slot 2) / work copy (slot 19) are REDUCE-carried across WMMA
  and must not share parked ACC (v126+). Tile-local soft/pv_soft (16/17) stay on
  the full VGPR pool — constraining those broke the SKIP=2 baseline.
  """
  if getattr(buf.arg, "slot", None) not in (2, 19): return VGPR
  uops = ctx.uops or []
