...
def _amd_flash_attention(o:UOp, q:UOp, cache:UOp, valid_kv_len:int|UOp, q_start:int|UOp|None=None) -> UOp:
  ...
  # ACC_SMALL stream: one ACC pack reused per TN column; copy each column to soft
  # before the next WMMA clobbers ACC (avoids multi-pack regalloc double-def).
  _acc_small = bool(getenv("AMD_WMMA_ACC_SMALL", 0))
  _stream_soft = bool(_acc_sep and _acc_small and TM // WMMA_ACC == 1 and getenv("AMD_FLASH_STREAM_SOFT", 1))
  if _stream_soft:
    ...
    for tn_i in range(TN):
      ...
      S_frag = S_cur.reshape(TM // WMMA_ACC, WMMA_ACC, TN).permute(0, 2, 1)[0, tn_i]
    ...
    qk_done = S_reg  # dependency carrier for later after()
    _soft_scale = False  # already scaled into soft
    _acc_sep = False  # S_reg is already the soft buffer
  else:
  ...
  if _stream_soft:
    S_masked = S_reg  # already soft+scaled
  ...
  if _stream_soft:
    # Same ACC-pack reuse: stream each TD column into pv_soft before the next WMMA.
    ...
    for td_i in range(TD):
      ...
      pv_frag = pv_cur.reshape(TM // WMMA_ACC, WMMA_ACC, TD).permute(0, 2, 1)[0, td_i]
      ...
      pv_stores.extend(pv_soft[ri, td_i].store(pv_cur[ri, td_i]) for ri in range(TM))
