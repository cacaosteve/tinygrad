...
def _amd_flash_attention(o:UOp, q:UOp, cache:UOp, valid_kv_len:int|UOp, q_start:int|UOp|None=None) -> UOp:
  ...
  # ACC_SMALL + ranged WMMA: keep ONE WMMA UOp (no pack double-def), always write
  # workspace column 0, and copy that column into soft[:, tn] inside the tn range
  # before the next iteration clobbers ACC.
  _stream_in_range = bool(_acc_sep and _acc_small and TM // WMMA_ACC == 1 and getenv("AMD_FLASH_STREAM_SOFT", 1))
  if _stream_in_range:
    ...
    S_frag = S_reg.reshape(TM // WMMA_ACC, WMMA_ACC, TN).permute(0, 2, 1)[tm1, 0]
    q_frag = Q_lds.reshape(WAVES_M, TM // WMMA_ACC, WMMA_M, D // WMMA_K, WMMA_K)[wave_m, tm1, lane_n, k_qk]
    k_frag = KV_lds_k.reshape(TN, WMMA_N, D // WMMA_K, WMMA_K)[tn1, lane_n, k_qk]
    wmma_st = S_frag.store(UOp.wmma(q_frag, k_frag, S_frag.after(k_qk), *WMMA_ARG))
    S_ws = S_reg.after(wmma_st)
    ri_c = UOp.range(TM, 240)
    q_idx = q_base + block_m * BLOCK_M + wave_m * WMMA_M + ri_c * LANES_PER_WAVE_M + lane_m
    k_idx = n_tile * BLOCK_N + tn1 * LANES_PER_WAVE_N + lane_n
    scaled = S_ws[ri_c, 0] * SCALE
    soft_st = S_soft[ri_c, tn1].store((k_idx <= q_idx).where(scaled, scaled.const_like(-math.inf)))
    qk_done = soft_st.end(ri_c, tm1, tn1).end(k_qk)
    S_reg = S_soft.after(qk_done)
    S_masked = S_reg
    _soft_scale = False
    _acc_sep_qk = False
  else:
  ...
  if _stream_in_range:
    ...
    pv_frag = pv_acc.reshape(TM // WMMA_ACC, WMMA_ACC, TD).permute(0, 2, 1)[tm2, 0]
    ...
    wmma_st = pv_frag.store(UOp.wmma(p_frag, v_frag, pv_frag.after(k_pv), *WMMA_ARG))
    pv_ws = pv_acc.after(wmma_st)
    ri_p = UOp.range(TM, 405)
    pv_st = pv_soft[ri_p, tn2].store(pv_ws[ri_p, 0])
    pv_acc = pv_soft.after(pv_st.end(ri_p, tm2, tn2).end(k_pv))
  else:
    k_pv, tm2, tn2 = UOp.range(BLOCK_N//WMMA_K, 400, AxisType.REDUCE), UOp.range(TM//WMMA_ACC, 401), UOp.range(TD, 402)
