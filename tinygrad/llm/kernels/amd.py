...
def _amd_flash_attention(o:UOp, q:UOp, cache:UOp, valid_kv_len:int|UOp, q_start:int|UOp|None=None) -> UOp:
  ...
  # ACC_SMALL single-workspace stream: reuse S_reg[:,0] / pv[:,0] as the only ACC pack,
  # copy each finished column into soft before the next WMMA (avoids multi-pack double-def).
  ...
  if _stream_ws:
    ...
    for tn_i in range(TN):
      ...
      # Always WMMA into column-0 workspace (one ACC pack).
    ...
    qk_done = S_reg
    _soft_scale = False
    _acc_sep_qk = False  # QK already in soft
  else:
