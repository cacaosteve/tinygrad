...
def _wmma_stores(out, outputs, tokens, accs, update, half, lane, wave, output_waves, swizzle_stores:bool=False):
  ...
  # Flat per-lane rows so isel can peel const offsets → ds_store/load_b128 fusion.
  n = len(flat_accs) * 8
  lds = UOp.placeholder((output_waves, 32 * n), dtypes.float32, slot=33, addrspace=AddrSpace.LOCAL)
  stores = [lds[wave, lane * n + a * 8 + i].store(acc.after(update)[i].load())
            for a, acc in enumerate(flat_accs) for i in range(8)]
  lds = lds.after(UOp.barrier(UOp.group(*stores)))
  def values(ai:int) -> tuple[UOp, ...]:
    own = tuple(lds[wave, lane * n + ai * 8 + i].load() for i in range(8))
    peer = tuple(lds[wave, (lane ^ 16) * n + ai * 8 + i].load() for i in range(8))
    low = half.eq(0)
    return tuple(low.where(own[i], peer[i+4]) if j == 0 else low.where(peer[i], own[i+4]) for i in range(4) for j in range(2))
  return [out[token, output].store(value) for ot,(output,output_accs) in enumerate(zip(outputs, accs))
