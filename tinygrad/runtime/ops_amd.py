...
SQTT_ITRACE_SE_MASK, SQTT_LIMIT_SE, SQTT_SIMD_SEL, SQTT_SA_SEL, SQTT_WGP_SEL, SQTT_TOKEN_EXCLUDE = \
  ContextVar("SQTT_ITRACE_SE_MASK", 0b11), ContextVar("SQTT_LIMIT_SE", 0), ContextVar("SQTT_SIMD_SEL", 0), \
  ContextVar("SQTT_SA_SEL", 0), ContextVar("SQTT_WGP_SEL", 0), ContextVar("SQTT_TOKEN_EXCLUDE", 0)
  ...
  def sqtt_start(self, buf0s:list[HCQBuffer]):
    ...
    if self.dev.target[0] == 9:
      ...
      for se in range(len(buf0s)):
        ...
        # NOTE: SQTT records detailed instruction tokens for one SIMD slot per SE.
        # For RGP to display instruction trace it has to see it on first SE. However ACE/MEC/whatever does the dispatching starting with second se,
        ...
        # The selector ContextVars default to the first slot and are only for controlled trace capture.
        cs_wtype = (1 << 6) if self.dev.target >= (12,0,0) else self.soc.SQ_TT_WTYPE_INCLUDE_CS_BIT
        self.wreg(self.gc.regSQ_THREAD_TRACE_MASK, wtype_include=cs_wtype, simd_sel=SQTT_SIMD_SEL.value,
                  wgp_sel=SQTT_WGP_SEL.value, sa_sel=SQTT_SA_SEL.value)
