...
from tinygrad.helpers import ALLOW_TF32, DEFAULT_FLOAT, DEFAULT_INT, TC_SELECT, TC_OPT, CCACHE, TracingKey, Context, panic, getenv
...
def do_devectorize(b:UOp):
  ...
  # quant _amd_load u32×4/×8 must stay packed for B128 isel (×8 → two B128 + s_clause).
  if b.op is Ops.LOAD and b.dtype is dtypes.uint32 and len(b.src) == 1 and b.src[0].op is Ops.SHRINK and \
      len(b.src[0].src) > 2 and b.src[0].src[2].op is Ops.CONST and b.src[0].src[2].val in (4, 8): return None
