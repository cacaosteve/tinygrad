#!/usr/bin/env python3
"""Benchmark the custom RDNA3 quantized-linear path used by tinygrad LLMs.

Examples:
  DEV=AMD:AMD CCACHE=0 python extra/bench_amd_qlinear.py --rows 8192 --cols 2048 --tokens 1
  DEV=AMD CCACHE=0 python extra/bench_amd_qlinear.py --rows 8192 --cols 2048 --tokens 32
"""
...
Q6_K, QK, BLOCK_BYTES = 14, 256, 210


def make_inputs(rows: int, cols: int, tokens: int) -> tuple[np.ndarray, np.ndarray]:
  blocks = rows * cols // QK
  qdata = (np.arange(blocks * BLOCK_BYTES, dtype=np.uint8) * np.uint8(37) + np.uint8(13))
  qdata.reshape(blocks, BLOCK_BYTES)[:, 208:210] = np.array([0.03125], dtype=np.float16).view(np.uint8)
...
def main() -> None:
  ...
  qdata, x_np = make_inputs(args.rows, args.cols, args.tokens)
  ...
  decoded = ggml_data_to_tensor(raw, args.rows * args.cols, Q6_K).reshape(args.rows, args.cols)
