...
class TestFloat4(unittest.TestCase):
  ...
  def count_float4(uops: list[UOp], n=4):
    return (len([uop for uop in uops if (uop.op is Ops.LOAD and uop.dtype == dtypes.float and uop.shape == (n,)) or
                 (uop.op is Ops.INS and getattr(uop.arg, "name", "") == "LOAD" and uop.dtype == dtypes.float and uop.max_numel() == n)]),
            len([uop for uop in uops if (uop.op is Ops.STORE and uop.src[1].dtype == dtypes.float and uop.shape == (n,)) or
                 (uop.op is Ops.INS and getattr(uop.arg, "name", "") == "STORE" and uop.src[2].dtype == dtypes.float
                  and uop.src[2].max_numel() == n)]))
  @staticmethod
